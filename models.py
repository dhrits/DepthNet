...
# My custom mobilenet-v2 architecture derived from the FastDepth work in the paper added at the end of this same file.
...
class MobileNetSkipAdd(nn.Module):
    def __init__(self, output_size=224, pretrained=True, interpolation='nearest', decoder_only=False):
        ...
        if self.decoder_only:
            # If decoder only, then only train
            # batch norm parameters in encoder
            for p in mobilenet.parameters():
                p.requires_grad = False
            for m in mobilenet.children():
    ...
    def forward(self, x):
        ...
        for i in range(1,6):
            ...
            x = layer(x)
            x = F.interpolate(x, scale_factor=2, mode=self.interpolation)
            if i==4:
                x = x + x1
            elif i==3:
                x = x + x2
...
# Custom adaptation of NNConv5 model based on mobilenetv2
class MobileNetV2SkipAdd(nn.Module):
    def __init__(self, output_size=224, pretrained=True, interpolation='nearest', decoder_only=False):
        ...
        for i, f in enumerate(mobilenet.features):
            setattr( self, 'conv{}'.format(i), f)
    ...
    def forward(self, x):
        ...
        # Decoding and skip (add) connections
        for i in range(1,6):
            layer = getattr(self, 'decode_conv{}'.format(i))
            x = layer(x)
            x = F.interpolate(x, scale_factor=2, mode=self.interpolation)
